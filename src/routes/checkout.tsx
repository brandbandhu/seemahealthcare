...
export const Route = createFileRoute("/checkout")({
  head: () => ({
    meta: [
      { title: "Checkout — Seema Healthcare" },
      { name: "description", content: "Multistep checkout with address, prescription, referral, delivery and payment options including cash on delivery." },
      { property: "og:title", content: "Checkout — Seema Healthcare" },
      { property: "og:description", content: "Secure demo checkout with UPI, cards, wallet and cash on delivery." },
    ],
  }),
  component: Checkout,
});
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      ...
      <ol className="mt-4 flex flex-wrap gap-2 text-xs">
        {stepNames.map((s, i) => (
          <li key={s} className={`rounded-full border px-3 py-1 font-medium ${i === step ? "border-primary bg-primary text-primary-foreground" : i < step ? "border-success text-success" : "text-muted-foreground"}`}>
            {i + 1}. {s}
          </li>
        ))}
      </ol>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div className="card-soft space-y-4 p-5">
          ...
          {step === 1 && (
            <>
              ...
              <Button asChild variant="outline"><Link to="/upload-prescription">Upload a new prescription</Link></Button>
            </>
          )}
          {step === 2 && (
            <>
              ...
              <div className="flex gap-2">
                ...
                <Button variant="outline" onClick={() => applyHospital(refCode)}>Validate</Button>
              </div>
          ...
          {step === 3 && (
            <>
              ...
              <RadioGroup value={delivery} onValueChange={setDelivery} className="space-y-2">
                {[["standard", "Standard delivery · 2–3 days"], ["express", "Express delivery · next day (₹99)"]].map(([v, l]) => (
          ...
          {step === 4 && (
            <>
              ...
              <RadioGroup value={payment} onValueChange={setPayment} className="space-y-2">
                {[["upi", "UPI"], ["card", "Debit / Credit card"], ["netbanking", "Net banking"], ["wallet", "Wallet"], ["cod", "Cash on delivery"]].map(([v, l]) => (
                  <div key={v} className="flex items-center gap-2">
                    <RadioGroupItem value={v!} id={`pay-${v}`} />
                    <Label htmlFor={`pay-${v}`} className="font-normal">{l}</Label>
                  </div>
                ))}
              </RadioGroup>
          ...
          {step === 5 && (
            <>
              ...
              <dl className="space-y-1 text-sm text-muted-foreground">
                <div><dt className="inline font-semibold text-foreground">Address: </dt><dd className="inline">{address}</dd></div>
                <div><dt className="inline font-semibold text-foreground">Delivery: </dt><dd className="inline">{delivery}</dd></div>
                <div><dt className="inline font-semibold text-foreground">Payment: </dt><dd className="inline">{payment.toUpperCase()}</dd></div>
                <div><dt className="inline font-semibold text-foreground">Items: </dt><dd className="inline">{totals.itemCount}</dd></div>
              </dl>
        ...
        <aside className="card-soft h-fit p-5 text-sm">
          ...
          <div className="flex justify-between"><span className="text-muted-foreground">Items</span><span>{totals.itemCount}</span></div>
          <div className="flex justify-between"><span className="text-muted-foreground">Savings</span><span className="text-success">{inr(totals.savings)}</span></div>
          <div className="mt-2 flex justify-between text-base font-extrabold"><span>Payable</span><span>{inr(totals.payable)}</span></div>
        </aside>
      </div>
    </div>
  );
