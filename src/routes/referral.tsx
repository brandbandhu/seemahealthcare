...
export const Route = createFileRoute("/referral")({
  head: () => ({
    meta: [
      { title: "Hospital Referral Rewards — Seema Healthcare" },
      { name: "description", content: "Enter your hospital referral code to earn signup points, order discounts and faster prescription review." },
      { property: "og:title", content: "Hospital Referral Rewards — Seema Healthcare" },
      { property: "og:description", content: "100 signup points, up to 7% off eligible orders, 1 point = ₹1." },
    ],
  }),
  component: Referral,
});
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <p className="text-sm text-muted-foreground">
        1 point = ₹1 · redeem up to 10% of any cart · 100-point signup bonus for referred customers.
      </p>

      <div className="card-soft mt-6 flex flex-wrap items-end gap-3 p-5">
        <div className="min-w-0 flex-1">
          <label htmlFor="ref" className="text-sm font-semibold">Enter your hospital referral code</label>
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        {hospitals.map((h) => (
          <article key={h.code} className="card-soft p-5">
            ...
            <p className="text-xs text-muted-foreground">{h.city} · Code {h.code}</p>
            <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
              <li>Referral discount: {h.discount}%</li>
              <li>Points multiplier: {h.multiplier}x</li>
              <li>Minimum order: ₹{h.minOrder} · Max discount: ₹{h.maxDiscount}</li>
            </ul>
          </article>
        ))}
      </div>
      ...
      <div className="card-soft mt-3 overflow-x-auto">
        <table className="w-full min-w-[640px] text-sm">
          <thead className="border-b text-left text-xs uppercase text-muted-foreground">
            <tr>{["Transaction", "Date", "Description", "Credit", "Debit", "Balance"].map((h) => <th key={h} className="p-3">{h}</th>)}</tr>
          </thead>
          <tbody>
            {ledger.map((r) => (
