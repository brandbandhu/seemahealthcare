...
export const Route = createFileRoute("/upload-prescription")({
  head: () => ({
    meta: [
      { title: "Upload Prescription — Seema Healthcare" },
      { name: "description", content: "Upload a doctor's prescription as JPG, PNG or PDF and receive a pharmacist-reviewed order value before you pay." },
      { property: "og:title", content: "Upload Prescription — Seema Healthcare" },
      { property: "og:description", content: "Pharmacist review in 30–60 minutes, then an itemised quotation." },
    ],
  }),
  component: UploadRx,
});
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div>
          ...
          <form onSubmit={submit} className="mt-5 space-y-5">
                ...
                <ul className="mt-3 w-full space-y-1 text-left text-xs">
                  {files.map((f, i) => (
                    <li key={`${f}-${i}`} className="flex items-center justify-between rounded-lg bg-muted px-3 py-2">
                      <span className="flex min-w-0 items-center gap-2 truncate"><FileUp className="h-3.5 w-3.5 shrink-0" />{f}</span>
                      <button type="button" className="text-destructive" onClick={() => setFiles((p) => p.filter((_, x) => x !== i))}>Remove</button>
                    </li>
                  ))}
                </ul>
              )}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <select id="hospital" className="mt-1 h-10 w-full rounded-md border bg-card px-3 text-sm">
                  ...
                  {hospitals.map((h) => <option key={h.code}>{h.name}</option>)}
                </select>
              </div>
            ...
            <Button type="submit" size="lg">Submit prescription</Button>
          </form>
        </div>
